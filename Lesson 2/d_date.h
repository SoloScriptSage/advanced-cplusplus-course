...
class date
{
   public:
      ...
      void writeShortDate () const; // output the date in the format "MM/DD/YYYY"
      void writeLongDate () const; // output the date in the format "month day, year"
      void incrementDate(int ndays); // add ndays days to the date. precondition: 0 <= ndays <= 365
      int numberOfDays() const; // return the number of days into the year
      int getMonth() const; // return the month as integer value 1 to 12
      int getDay() const; // return day of the month
      int getYear() const; // return the year
      void setMonth(int mm); // update the month // precondition: 1 <= mm <= 12
      void setDay(int dd); // update the day // precondition: 1 <= dd <= daysInMonth()
      void setYear(int yyyy); // update the year // precondition:  if the date is February 29, // yyyy must be a leap year
      int daysInMonth() const; // return number of days in the month
...
bool date::operator>(const date& d) const{
   if(year > d.year) return true;
      ...
      else if(month == d.month) return day > d.day;
   }
   return false;
}
...
void date::setYear(int yyyy)
{
   ...
   if (month == 2 && day == 29 && !isLeapYear())
		throw dateError("date setYear():", year, "Not a leap year. February 29 invalid");
}
